...
pub fn config_dir() -> Result<PathBuf, MetadataError> {
    let dirs = directories::ProjectDirs::from("org", "lyraos", "sulafat")
        .ok_or(MetadataError::NoConfigDir)?;
...
impl Metadata {
    ...
    fn save_to(&self, path: &std::path::Path) -> Result<(), MetadataError> {
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).map_err(|e| MetadataError::io(parent, e))?;
        }
        let contents = toml::to_string_pretty(self)?;
        crate::atomic::write(path, contents.as_bytes()).map_err(|e| MetadataError::io(path, e))
    }
}
...
mod tests {
    ...
    fn set_and_get_round_trip_through_toml() {
        ...
        assert_eq!(
            parsed.get("prod").unwrap().color.as_deref(),
            Some("#e01b24")
        );
    }
    ...
    fn groups_are_sorted_and_deduplicated() {
        ...
        assert_eq!(
            meta.groups(),
            vec!["Homolog".to_string(), "Prod".to_string()]
        );
    }
    ...
    #[cfg(unix)]
    #[test]
    fn save_is_atomic_restricted_and_refuses_symlinks() {
        ...
        assert!(meta.save_to(&path).is_err());
